...
notificationMainController.controller('monitorSettingController', ['$scope', '$http','$state', '$rootScope', '$ionicPopup',
    function ($scope, $http,$state, $rootScope,$ionicPopup) {
        ...
        $scope.saveDeviceID= function(id) {
            $http.get("http://"+$rootScope.hostname+"/MS4SF/public/index.php/notificationMonitor/" +id).success(function (data) {
        ...
        $scope.updateMonitor = function(){
            $http.post("http://"+$rootScope.hostname+"/MS4SF/public/index.php/updateNotificationMonitor",$scope.notificationDevice).success(function(data){
                $ionicPopup.alert({
                    title: 'Success!',
...
notificationMainController.controller('NotificationSettingController', ['$scope', '$http','$state', '$rootScope', '$ionicPopup',
    function ($scope, $http,$state, $rootScope,$ionicPopup) {
        ...
        $http.get("http://"+$rootScope.hostname+"/MS4SF/public/index.php/getTaskNotice?id=" + $rootScope.User.id).success(function (data) {
            if(data.length!=0) {
                $scope.noticeTask = data;
            }
        });
        $scope.changeTaskStatus = function(status){
            $http.get("http://"+$rootScope.hostname+"/MS4SF/public/index.php/changeTaskNotice?id=" +$rootScope.User.id+"&status="+status).success(function (data) {
                console.log(data);
            });
        };
        $http.get("http://"+$rootScope.hostname+"/MS4SF/public/index.php/notification?id=" + $rootScope.User.id).success(function (data) {
            if(data.length!=0) {
                $scope.noticeList = data;
            }
        });
        $scope.changeStatus = function(id,status){
            $http.get("http://"+$rootScope.hostname+"/MS4SF/public/index.php/changeNotice?id=" +id+"&status="+status+"&user_id="+$rootScope.User.id).success(function (data) {
                console.log(data);
            });
        };
        $scope.changeTaskNotificationStatus = function(id,status){
            $http.get("http://"+$rootScope.hostname+"/MS4SF/public/index.php/changeNotice?id=" +id+"&status="+status).success(function (data) {
                console.log(data);
            });
        };
        $scope.deleteNotification =function(id){
            ...
            confirmPopup.then(function(res) {
                if(res) {
                    $http.get("http://"+$rootScope.hostname+"/MS4SF/public/index.php/deleteNotice?id=" + id + "&status=" + status).success(function (data) {
...
notificationMainController.controller('editNotificationController', ['$scope', '$http','$state', '$rootScope', '$ionicPopup','$stateParams',
    function ($scope, $http,$state, $rootScope,$ionicPopup,$stateParams) {
        ...
        if($rootScope.User==null){
            ...
            $http.get("http://"+$rootScope.hostname+"/MS4SF/public/index.php/notificationMonitor/" +$stateParams.id).success(function (data) {
                if (data.length != 0&&data !="") {
                    $scope.notificationDevice = data;
                }else{
                    $scope.notificationDevice.deviceID =id;
