...
deviceMainController.controller('PlotWithDeviceController',['$scope','$http','$state', '$rootScope','deviceService','$ionicPopup',
    function ($scope, $http,$state, $rootScope,deviceService,$ionicPopup) {
        ...
        $http.get("http://"+$rootScope.hostname+"/MS4SF/public/index.php/device/"+$rootScope.plotId+"/edit").success(function (data) {
            $scope.devices = data;
        });
        ...
        $scope.deleteDevice = function(id){
            ...
            if (answer) {
                $http.get("http://"+$rootScope.hostname+"/MS4SF/public/index.php/device/destroy/"+id).success(function () {
                    $ionicPopup.alert({
                        title: 'Success',
                        template: 'success'
                    });
...
deviceMainController.controller('sensorMonitoringSummaryController',['$scope', '$interval','$http','$location', '$rootScope',
    ...
    var id =$rootScope.DeviceId;
        ...
        $http.get("http://"+$rootScope.hostname+"/MS4SF/public/index.php/sensor/"+id).success(function(data){
            $scope.SensorID = data.id;
            $rootScope.showSensor = true;
            $http.get("http://"+$rootScope.hostname+"/MS4SF/public/index.php/lightSummary/"+$scope.SensorID).success(function(data){
                $scope.light = data;
            });
            $http.get("http://"+$rootScope.hostname+"/MS4SF/public/index.php/humiditySummary/"+$scope.SensorID).success(function(data){
                $scope.humidity = data;
            });
            $http.get("http://"+$rootScope.hostname+"/MS4SF/public/index.php/soilMoistureSummary/"+$scope.SensorID).success(function(data){
                $scope.soilMoisture = data;
            });
            $http.get("http://"+$rootScope.hostname+"/MS4SF/public/index.php/temperatureSummary/"+$scope.SensorID).success(function(data){
                $scope.temperature = data;
            });
            $http.get("http://"+$rootScope.hostname+"/MS4SF/public/index.php/daily/"+$scope.SensorID).success(function(data){
                $scope.hours = data;
            });
            $http.get("http://"+$rootScope.hostname+"/MS4SF/public/index.php/currentEnvironmentValue/"+$scope.SensorID).success(function(data){
                $scope.current = data;
            });
        });
    $interval(function(){
...
deviceMainController.controller('sensorMonitoringWeeklySummaryController',['$scope','$routeParams','$route','$http','$location', '$rootScope',
    function ($scope,$routeParams,$route, $http,$location, $rootScope) {
        ...
        $http.get("http://"+$rootScope.hostname+"/MS4SF/public/index.php/sensor/"+id).success(function(data){
            ...
            $http.get("http://"+$rootScope.hostname+"/MS4SF/public/index.php/lightWeeklySummary/"+$scope.SensorID).success(function(data){
                $scope.light = data;
            });
            $http.get("http://"+$rootScope.hostname+"/MS4SF/public/index.php/humidityWeeklySummary/"+$scope.SensorID).success(function(data){
                $scope.humidity = data;
            });
            $http.get("http://"+$rootScope.hostname+"/MS4SF/public/index.php/soilMoistureWeeklySummary/"+$scope.SensorID).success(function(data){
                $scope.soilMoisture = data;
            });
            $http.get("http://"+$rootScope.hostname+"/MS4SF/public/index.php//"+$scope.SensorID).success(function(data){
                $scope.temperature = data;
            });
            $http.get("http://"+$rootScope.hostname+"/MS4SF/public/index.php/weekly/"+$scope.SensorID).success(function(data){
                $scope.dailys = data;
        ...
        $scope.saveFarmID = function(id) {
           for(var i=0;i<$scope.farms.length;i++){
               if($scope.farms[i].id==id){
                   ...
                   $http.get("http://"+$rootScope.hostname+"/MS4SF/public/index.php/plot/" + id + "/edit").success(function (data) {
                       $scope.plots = data;
                       $scope.loadPlot = true;
                   });
               }
        ...
        $scope.savePlotID = function(id,name) {
            for(var i=0;i<$scope.plots.length;i++) {
                if ($scope.plots[i].id == id) {
                    ...
                    $http.get("http://"+$rootScope.hostname+"/MS4SF/public/index.php/device/" + id + "/edit").success(function (data) {
                        $scope.devices = data;
